        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($recentlySigned->isNotEmpty())
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-6">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-gray-700 mb-4">✅ Baru Ditandatangani</h3>
                        <div class="space-y-2">
                            @foreach($recentlySigned as $form)
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                                    <div>
                                        <p class="font-medium text-gray-800">{{ $form->no_terima_sampel ?? $form->lhp_number ?? '-' }}</p>
                                        <p class="text-sm text-gray-500">{{ $form->customer_name }} &middot;
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $form->status === 'selesai' ? 'bg-green-100 text-green-700' : 'bg-teal-100 text-teal-700' }}">
                                                {{ $form->status === 'selesai' ? 'Selesai' : 'Menunggu Kirim' }}
                                            </span>
                                        </p>
                                    </div>
                                    <a href="{{ route('kepala-upa.show', $form) }}"
